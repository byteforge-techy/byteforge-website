  ...
  return (
    <>
      <style>{`
        ...
        .nav-link-item:hover { color: ${BLUE} !important; }
        .nav-cta-btn:hover { background: ${BLUE} !important; transform: translateY(-1px); }
        .hamburger-bar { transition: all 0.3s; }
        .mobile-nav-link:hover { color: ${BLUE} !important; }
      `}</style>
      <nav style={{
          ...
          {NAV_LINKS.map(({ label, path }) => (
            <li key={label}
              ...
              style={{
                ...
                color: isActive(path) ? BLUE : TEXT_MUTED,
                fontFamily: "'Outfit', sans-serif",
                borderBottom: isActive(path) ? `2px solid ${BLUE}` : "2px solid transparent",
                paddingBottom: 2,
              }}
        ...
        <div style={{ display: "none", flexDirection: "column", gap: 5, cursor: "pointer" }}
          ...
          {[0, 1, 2].map((i) => (
            <div key={i} className="hamburger-bar" style={{
              width: 24, height: 2, background: DARK,
      ...
      {menuOpen && (
        <div style={{
          ...
          <button
            style={{
              marginTop: 16, background: DARK, color: "#fff", border: "none",
