...
export default function Contact() {
  ...
  const contactCards = [
    ...
    { icon:"🌐", label:"Website", value:"byteforgetechnology.com", c:"#7c3aed" },
    ...
    <div style={{ background:"#fff", ...fontHead, overflowX:"hidden" }}>
      ...
      <section style={{ position:"relative", background:`linear-gradient(135deg, ${DARK} 0%, #14172a 55%, #1a2348 100%)`, color:"#fff", paddingTop:150, paddingBottom:100, overflow:"hidden", textAlign:"center" }}>
        ...
        <div style={{ ...wrap, position:"relative" }}>
          <Reveal><SectionLabel color="#bcd0ff">Get in Touch</SectionLabel></Reveal>
          ...
          <Reveal delay={0.12}><p style={{ fontSize:"clamp(16px,2vw,19px)", lineHeight:1.7, color:"#aab4cf", maxWidth:620, margin:"0 auto" }}>Tell me about your project. I respond personally within 24 hours with honest, practical advice.</p></Reveal>
        </div>
      </section>
      ...
      <section style={{ background:"#fff", padding:"0", marginTop:-50, position:"relative", zIndex:2 }}>
        <div style={{ ...wrap }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))", gap:0, background:"#fff", borderRadius:20, boxShadow:"0 20px 60px rgba(0,0,0,0.1)", overflow:"hidden", border:"1px solid #eee" }}>
            {contactCards.map((c,i) => (
              <div key={i} style={{ padding:"32px 28px", textAlign:"center", borderRight:i<contactCards.length-1?"1px solid #f0f0f0":"none" }}>
                ...
                <div style={{ fontSize:13, color:"#999", fontWeight:600, textTransform:"uppercase", letterSpacing:"1px", marginBottom:6 }}>{c.label}</div>
      ...
      <section style={{ background:"#fff", padding:"80px 0 100px" }}>
        <div style={{ ...wrap, display:"grid", gridTemplateColumns:"1.3fr 1fr", gap:48, alignItems:"start" }}>
          {/* FORM */}
          <Reveal>
            <div style={{ background:"#fff", border:"1px solid #ececec", borderRadius:20, padding:"40px", boxShadow:"0 8px 30px rgba(0,0,0,0.05)" }}>
              {submitted ? (
                <div style={{ textAlign:"center", padding:"40px 0" }}>
                  ...
                  <p style={{ fontSize:16, color:"#666", lineHeight:1.6 }}>Thanks, <strong>{form.name}</strong>! I'll reach out to <strong>{form.email}</strong> within 24 business hours.</p>
                </div>
              ) : (
                <>
                  <h3 style={{ ...fontHead, fontSize:24, fontWeight:800, color:INK, margin:"0 0 6px" }}>Tell Me About Your Project</h3>
                  <p style={{ fontSize:15, color:"#888", margin:"0 0 28px" }}>Fields marked * are required.</p>
          ...
          <Reveal delay={0.1}>
            <div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                {faqs.map((f,i) => (
                  <div key={i} onClick={()=>setOpenFaq(openFaq===i?null:i)} style={{ background:"#f8f9fc", border:"1px solid #ececec", borderRadius:14, padding:"18px 20px", cursor:"pointer" }}>
                    ...
                    {openFaq===i && <p style={{ fontSize:14.5, lineHeight:1.6, color:"#666", margin:"12px 0 0" }}>{f.a}</p>}
                  </div>
                ))}
              </div>
            </div>
