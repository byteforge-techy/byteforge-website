    ...
    <div style={{ background:"#fff", ...fontHead, overflowX:"hidden" }}>
      ...
      <section style={{ position:"relative", background:`linear-gradient(135deg, ${DARK} 0%, #14172a 55%, #1a2348 100%)`, color:"#fff", paddingTop:150, paddingBottom:90, overflow:"hidden", textAlign:"center" }}>
        ...
        <div style={{ ...wrap, position:"relative" }}>
          <Reveal><SectionLabel color="#bcd0ff">Insights & Updates</SectionLabel></Reveal>
          <Reveal delay={0.06}>
            <h1 style={{ ...fontHead, fontSize:"clamp(36px,6vw,68px)", fontWeight:800, lineHeight:1.05, letterSpacing:"-2px", margin:"0 0 10px", color:"#fff" }}>
              The Byte Forge <span style={{ background:`linear-gradient(90deg,${BLUE},#7c3aed)`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Blog</span>
            </h1>
          </Reveal>
          <Reveal delay={0.12}><p style={{ fontSize:"clamp(16px,2vw,19px)", lineHeight:1.7, color:"#aab4cf", maxWidth:620, margin:"0 auto" }}>Engineering deep-dives, case studies, and thoughts on building software that lasts.</p></Reveal>
        </div>
      </section>

      {/* CONTENT */}
      <section style={{ background:"#f8f9fc", padding:"60px 0 100px" }}>
        <div style={{ ...wrap }}>
          {/* Category filters */}
          <div style={{ display:"flex", flexWrap:"wrap", gap:12, justifyContent:"center", marginBottom:48 }}>
            {categories.map((c,i) => (
              <button key={i} onClick={()=>setActiveCat(c)} style={{ ...fontHead, padding:"10px 24px", borderRadius:30, fontSize:14.5, fontWeight:600, cursor:"pointer", border:activeCat===c?"none":"1px solid #ddd", background:activeCat===c?INK:"#fff", color:activeCat===c?"#fff":"#555", transition:"all .2s" }}>{c}</button>
            ))}
          </div>

          {loading ? (
            <div style={{ textAlign:"center", color:"#888", padding:"60px 0" }}>Loading posts…</div>
            ...
            <>
              ...
              {featured && (
                <Reveal>
                  <div onClick={()=>navigate(`/blog/${featured.slug}`)} style={{ background:"#fff", borderRadius:20, overflow:"hidden", border:"1px solid #ececec", cursor:"pointer", display:"grid", gridTemplateColumns:"1.1fr 1fr", marginBottom:40, boxShadow:"0 8px 30px rgba(0,0,0,0.06)", transition:"transform .3s, box-shadow .3s" }}
                    ...
                    <div style={{ padding:"44px 40px", display:"flex", flexDirection:"column", justifyContent:"center" }}>
                      ...
                      <p style={{ fontSize:16, lineHeight:1.65, color:"#666", margin:"0 0 20px" }}>{featured.excerpt}</p>
                      <div style={{ display:"flex", alignItems:"center", gap:16, fontSize:13.5, color:"#999" }}>
                        <span>{fmtDate(featured.publishedAt || featured.createdAt)}</span>
                        {featured.views!=null && <span>· {featured.views} views</span>}
                      </div>
                    </div>
                    <div style={{ background:`linear-gradient(135deg, ${catColor(featured.category).accent}, #1e3a5f)`, display:"flex", alignItems:"center", justifyContent:"center", minHeight:280, fontSize:80 }}>
                ...
                {rest.map((post,i) => {
                  ...
                  return (
                    <Reveal key={post.id||i} delay={0.03+(i%3)*0.06}>
                      <div onClick={()=>navigate(`/blog/${post.slug}`)} style={{ background:"#fff", borderRadius:18, overflow:"hidden", border:"1px solid #ececec", cursor:"pointer", height:"100%", boxShadow:"0 4px 20px rgba(0,0,0,0.04)", transition:"transform .3s, box-shadow .3s" }}
                        ...
                        <div style={{ height:150, background:`linear-gradient(135deg, ${c.accent}, #1e3a5f)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:48, position:"relative" }}>
                          {c.icon}
                        ...
                        <div style={{ padding:"24px" }}>
                          ...
                          <p style={{ fontSize:14.5, lineHeight:1.6, color:"#666", margin:"0 0 16px", display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical", overflow:"hidden" }}>{post.excerpt}</p>
