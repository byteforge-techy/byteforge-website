  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:9999, background:DARK, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", opacity:fadeOut?0:1, transition:"opacity 0.6s ease", pointerEvents:fadeOut?"none":"all" }}>
      ...
      <div style={{ width:200, height:3, background:"#1e1e1e", borderRadius:10, overflow:"hidden" }}>
        <div style={{ height:"100%", borderRadius:10, background:`linear-gradient(90deg,${BLUE},${BLUE_LIGHT})`, width:`${progress}%`, transition:"width 0.1s linear" }}/>
      </div>
      <div style={{ color:TEXT_ON_DARK_SOFT, fontSize:13, fontFamily:"'Outfit',sans-serif", marginTop:16 }}>
...
export default function Home() {
  ...
  const services = [
    ...
    { icon:"🌐", title:"Web Platforms", desc:"Modern, responsive web apps with React and Angular — fast, scalable, and built to last.", color:BLUE_DARK },
    ...
    { icon:"☁️", title:"Cloud & DevOps", desc:"AWS deployment, CI/CD pipelines, and infrastructure that keeps your product reliable.", color:AMBER_LIGHT },
    { icon:"🏥", title:"Healthcare Solutions", desc:"Diagnostic booking, report tracking, and health apps — a domain I know deeply.", color:BLUE_LIGHT },
    { icon:"🛒", title:"E-Commerce & POS", desc:"Shopping platforms, marketplaces, and point-of-sale systems with real-time inventory.", color:AMBER_DARK },
  ];
    ...
    <div style={{ background:"#fff", fontFamily:"'Outfit',sans-serif", overflowX:"hidden" }}>
      ...
      <section style={{ position:"relative", background:`linear-gradient(135deg, ${DARK} 0%, #14172a 55%, #1a2348 100%)`, color:"#fff", paddingTop:140, paddingBottom:110, overflow:"hidden" }}>
        ...
        <div style={{ ...wrap, position:"relative", textAlign:"center" }}>
          <Reveal>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(37,99,235,0.15)", border:`1px solid ${BLUE}55`, color:BADGE_TEXT, padding:"8px 20px", borderRadius:30, fontSize:14, fontWeight:600, marginBottom:32 }}>
          ...
          <Reveal delay={0.12}>
            <div style={{ ...fontHead, fontSize:"clamp(34px,6vw,76px)", fontWeight:800, lineHeight:1.05, letterSpacing:"-2px", marginBottom:8, minHeight:"1.1em" }}>
              <span style={{ background:`linear-gradient(90deg,${BLUE},${BLUE_LIGHT})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>{typed}</span>
          ...
          <Reveal delay={0.2}>
            <p style={{ fontSize:"clamp(16px,2vw,19px)", lineHeight:1.7, color:TEXT_ON_DARK, maxWidth:720, margin:"0 auto 40px" }}>
              {heroSub}
            </p>
          </Reveal>
          <Reveal delay={0.24}>
      ...
      <section style={{ background:"#fff", padding:"0", marginTop:-50, position:"relative", zIndex:2 }}>
        <div style={{ ...wrap }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:0, background:"#fff", borderRadius:20, boxShadow:"0 20px 60px rgba(0,0,0,0.1)", overflow:"hidden", border:`1px solid ${BORDER}` }}>
            {[
              { n:statsProjects, s:"+", l:"Live Projects", c:BLUE },
              { n:statsDelivery, s:"+", l:"Years Experience", c:AMBER },
              { n:statsClients, s:"+", l:"Happy Clients", c:BLUE_DARK },
              { n:statsDomains, s:"", l:"Industries Served", c:AMBER_LIGHT },
            ].map((st, i) => (
              <div key={i} style={{ padding:"40px 24px", textAlign:"center", borderRight:i<3?`1px solid ${BORDER}`:"none" }}>
                <div style={{ ...fontHead, fontSize:48, fontWeight:800, color:st.c, lineHeight:1 }}>
                  <Counter end={st.n} suffix={st.s} />
                </div>
                <div style={{ fontSize:15, color:TEXT_MUTED, marginTop:8, fontWeight:500 }}>{st.l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background:"#fff", padding:"100px 0 90px" }}>
        <div style={{ ...wrap, textAlign:"center" }}>
          ...
          <Reveal delay={0.1}>
            <p style={{ fontSize:18, lineHeight:1.7, color:TEXT_MUTED, maxWidth:720, margin:"0 auto 56px" }}>{aboutBody}</p>
          </Reveal>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:24 }}>
            {[
              ...
              { icon:"🤝", t:"End-to-End Ownership", d:"Architecture, development, deployment — handled personally with full accountability at every step.", c:BLUE_DARK },
            ].map((p, i) => (
              <Reveal key={i} delay={0.1 + i*0.08}>
                <div style={{ background:"#fff", border:`1px solid ${BORDER}`, borderRadius:18, padding:"36px 28px", textAlign:"left", height:"100%", boxShadow:"0 4px 20px rgba(0,0,0,0.04)", transition:"transform 0.3s, box-shadow 0.3s" }}
                  ...
                  <p style={{ fontSize:15.5, lineHeight:1.65, color:TEXT_MUTED, margin:0 }}>{p.d}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section style={{ background:`linear-gradient(160deg, ${INK}, #1a1f3a)`, padding:"100px 0", position:"relative" }}>
        ...
        <div style={{ ...wrap, position:"relative", textAlign:"center" }}>
          ...
          <Reveal delay={0.1}>
            <p style={{ fontSize:18, color:TEXT_ON_DARK, maxWidth:640, margin:"0 auto 56px" }}>{servicesTag}</p>
          </Reveal>
            ...
            {services.map((s, i) => (
              <Reveal key={i} delay={0.05 + i*0.06}>
                <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:18, padding:"32px 28px", height:"100%", backdropFilter:"blur(10px)", transition:"transform 0.3s, border-color 0.3s, background 0.3s" }}
                  ...
                  <p style={{ fontSize:15, lineHeight:1.65, color:TEXT_ON_DARK, margin:0 }}>{s.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <section style={{ background:BG_ALT, padding:"100px 0" }}>
          <div style={{ ...wrap }}>
            <div style={{ textAlign:"center" }}>
              ...
              <Reveal delay={0.1}>
                <p style={{ fontSize:18, color:TEXT_MUTED, maxWidth:600, margin:"0 auto 56px" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(340px,1fr))", gap:24 }}>
              {featured.map((p, i) => (
                <Reveal key={p.id || i} delay={0.05 + i*0.06}>
                  <div onClick={() => navigate("/portfolio")} style={{ background:"#fff", borderRadius:18, overflow:"hidden", border:`1px solid ${BORDER}`, cursor:"pointer", height:"100%", boxShadow:"0 4px 20px rgba(0,0,0,0.04)", transition:"transform 0.3s, box-shadow 0.3s" }}
                    ...
                    <div style={{ padding:"24px" }}>
                      <p style={{ fontSize:15, lineHeight:1.6, color:TEXT_MUTED, margin:"0 0 16px", display:"-webkit-box", WebkitLineClamp:3, WebkitBoxOrient:"vertical", overflow:"hidden" }}>
                        {p.description}
                      </p>
                      {p.techStack && (
      ...
      <section style={{ background:`linear-gradient(135deg, ${INK}, #1a2348)`, padding:"90px 0", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ ...wrap, position:"relative", textAlign:"center" }}>
          ...
          <Reveal delay={0.08}>
            <p style={{ fontSize:19, color:TEXT_ON_DARK, maxWidth:560, margin:"0 auto 36px" }}>{ctaSub}</p>
          </Reveal>
          <Reveal delay={0.16}>
